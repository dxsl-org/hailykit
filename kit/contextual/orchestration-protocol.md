...
2. **Reports Path**: `{work_context}/.agents/reports/` for that project
3. **Plans Path**: `{work_context}/.agents/` for that project

**Example:**
```
Task prompt: "Fix parser bug.
Work context: /path/to/project-b
Reports: /path/to/project-b/.agents/reports/
Plans: /path/to/project-b/.agents/"
```
...
|--------|---------|-------------------|
| **DONE** | Task completed successfully, verification commands ran and passed | Proceed to next step (review, next task) |
...
DONE without a Verification line is treated as DONE_WITH_CONCERNS. Verification must name the command and its outcome — not "tests passed" but "npm test: 59 refs checked, 0 errors".
...
For multi-session parallel collaboration, use Claude Code's native Agent Team feature
(teammates coordinate via `SendMessage`; membership is detected from
`~/.claude/teams/{team-name}/config.json`). This is a platform capability, not a
HailyKit skill — no `/hc-*` or `/hl-*` invocation is needed to activate it. Once a
session is operating as a teammate, the team-coordination-rules.md contextual file
governs file ownership, communication protocol, and shutdown handling.
