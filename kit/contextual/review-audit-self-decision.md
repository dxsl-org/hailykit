...
2. **Categorize:**
   ...
   - Must confirm first: anything touching user's explicit answer — thresholds, scope, library, schema, phase, feature inclusion/exclusion.
...
Rule of thumb: if unsure whether a cut reverses user intent, ask. Cost of 1 clarifying question is far less than the cost of silent regression at demo.
...
- Plan refs belong in `.agents/…/phase-XX-*.md` and PR descriptions, not in code.
