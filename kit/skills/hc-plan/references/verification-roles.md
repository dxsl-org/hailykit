...
**Loaded by:** haily-planner (self-verify during creation), validate (external verify after creation), red-team (evidence-backed adversarial review).

**Principle:** `user asks → scout → haily-planner writes → audit-verify → report` instead of `user asks → haily-planner writes → report done`.
...
- **Validate — Verification Pass:** Run tier-appropriate roles after reading plan, before interview questions. FAILED findings become additional interview topics.
