...
**Skeleton:** Introduction (gap → question) → Methods (replicable detail) → Results (report only) → Discussion (interpret, compare to prior work, limitations) → References. This is the dominant norm for original-research journal articles.
...
**Unit:** one Build unit = one IMRaD section (Introduction, Methods, Results, Discussion each close as a unit).
