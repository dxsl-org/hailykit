...
Visual verification of frontend implementations using Chrome MCP (Claude Chrome Extension) or `chrome-devtools (Puppeteer)` skill fallback.
...
When Chrome MCP is not configured, use `chrome-devtools (Puppeteer)` skill (Puppeteer with bundled Chromium):
...
If `chrome-devtools (Puppeteer)` skill is also unavailable, skip visual verification and note in report:
