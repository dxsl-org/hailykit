...
def resolve_key(var_name):
    ...
    try:
        from resolve_env import resolve_env
        return resolve_env(var_name, skill='document-extract')
    except ImportError:
...
def find_api_key():
    ...
    if api_key:
        ...
        print_info("python ~/.claude/scripts/resolve_env.py GEMINI_API_KEY --skill document-extract --verbose")
        return api_key
...
def check_directory_structure():
    ...
    required_files = [
        ...
        ('document_converter.py', script_dir / 'document_converter.py'),
        ('media_optimizer.py', script_dir / 'media_optimizer.py'),
    ]
...
def main():
    ...
    if all_passed:
        ...
        print("  • Try: python scripts/document_converter.py --help")
        print("\nExamples:")
        print("  # Convert PDF to markdown")
        print("  python scripts/document_converter.py input.pdf --output output.md")
        print("\n  # Preflight resize large image")
