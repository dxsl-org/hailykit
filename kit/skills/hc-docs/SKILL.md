...
**Related:** `{skill:hc-review}`, `{skill:hc-new}`, `{skill:hl-ocr}` — mass conversion of a scanned PDF/image corpus (tiered cost, sample verification); use `extract` here for a single external doc or project docs
