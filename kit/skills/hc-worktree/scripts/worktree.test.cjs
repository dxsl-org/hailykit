...
const MONOREPO_DIR = '/home/kai/HailyKit';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'hailykit', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from hailykit submodule
  const submodulePath = '/home/kai/HailyKit/hailykit';
...
test('create with partial project match in monorepo', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'HailyKit-cli', 'Should find HailyKit-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'HailyKit' matches multiple projects
  const result = run('create HailyKit test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
