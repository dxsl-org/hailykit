...
Fast scout-diagnose-fix-verify cycle for simple issues (single file, clear error, lint/type).
...
Activate `{skill:hc-debug}`. Activate `{skill:hl-reasoning}` for structured analysis.
...
**Prevention (abbreviated for Simple):**
...
|------|------------------|
| 1 | `{skill:hc-scout}` (minimal) or direct file read |
| 2 | `{skill:hc-debug}`, `{skill:hl-reasoning}` |
| 3 | Parallel `Bash` for verification |
| 4 | `haily-reviewer` subagent |
| 5 | `haily-git-manager` subagent |

**Extra:** `{skill:hl-context-engineering}` if dealing with AI/LLM code
...
- Pre-fix state capture is STILL mandatory (even for simple fixes)
