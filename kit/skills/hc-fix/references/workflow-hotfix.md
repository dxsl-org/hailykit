...
Emergency fix for active production incidents. Speed is secondary to surgical precision — a bad hotfix compounds the incident. Every step has a hard time cap.

**Activation:** `{skill:hc-fix} [description] --hotfix`
...
A hotfix is a debt instrument. These must happen:
