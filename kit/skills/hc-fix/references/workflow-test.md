...
Use `haily-tester` agent. Fix all syntax errors before running tests.
...
Use `haily-debugger` agent for root cause analysis.
...
Use `haily-planner` agent for fix strategy.
...
Use `haily-tester` agent. If tests still fail → keep T5 `in_progress`, loop back to Step 2.

`TaskUpdate(T5, status="completed")`

### Step 6: Review
`TaskUpdate(T6, status="in_progress")`
Use `haily-reviewer` agent.
