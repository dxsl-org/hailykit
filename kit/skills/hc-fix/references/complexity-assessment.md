...
### Parallel (→ multiple haily-implementor agents) — Use Task Trees
...
**Task usage:** Create separate task trees per independent issue (each with scout+diagnose+fix+verify). Spawn `haily-implementor` agent per tree. See `references/task-orchestration.md`.
