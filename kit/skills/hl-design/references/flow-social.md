...
1. **``scripts/ui-ux/search.py``** — Extract brand colors, fonts, voice from user's project
2. **``scripts/ui-ux/search.py``** — Get design tokens (spacing, typography scale, color palette)
3. **Randomly invoke ONE of:** ``scripts/ui-ux/search.py`` OR ``scripts/ui-ux/search.py`` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
...
Save report to `.agents/reports/` with naming pattern from session hooks.
...
- Ensure reports are in `.agents/reports/` with correct naming
