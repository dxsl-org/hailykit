...
- Pre-landing review (from `{skill:hc-ship}` pipeline)
...
## Integration with {skill:hc-ship}
...
## Integration with {skill:hc-review}

When invoked as part of standard code review, the checklist augments (not replaces) the existing scout → review → fix → verify pipeline. Checklist findings are merged with haily-reviewer's own findings.
