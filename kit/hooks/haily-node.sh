...
# Runs HailyKit Node hooks without turning a missing Node.js runtime into a
...
warning_marker="$warning_dir/hailykit-haily-node.warned"
if [ ! -e "$warning_marker" ]; then
  {
    printf '%s\n' "[HailyKit] Node.js 18+ was not found on PATH, so HailyKit hooks were skipped."
    printf '%s\n' "[HailyKit] Install Node.js from https://nodejs.org/ and restart Claude Code."
  } >&2
