#!/usr/bin/env node
/**
 * error.cjs - Rich, actionable error messages for scout-block
 *
...
function formatConfigPath(claudeDir, configPath) {
  ...
  if (claudeDir) return path.join(claudeDir, 'haily.json');
  return '.claude/haily.json';
}
...
function formatBlockedError(details) {
  ...
  const lines = [
    ...
    '',
    `  ${colorize('To allow, add to', 'blue')} ${resolvedConfigPath} guard.allow:`,
    `    "${pattern}"`,
    '',
...
function formatMachineError(details) {
  ...
  return JSON.stringify({
    ...
    fix: `Add "${pattern}" to guard.allow in ${resolvedConfigPath} to allow this path`
  });
}
