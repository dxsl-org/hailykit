...
const POINTER_PATH = path.join('.agents', 'workflow-artifacts.json');
...
function findHarnessDirs(cwd) {
  const roots = [
    path.join(cwd, '.agents', 'reports', 'harness'),
...
function resolveArtifactDir({ cwd = process.cwd(), artifactDir = null, env = process.env } = {}) {
  ...
  const fromEnv = safeResolve(cwd, env.HL_WORKFLOW_ARTIFACT_DIR);
  if (fromEnv) return { artifactDir: fromEnv, source: 'env', reasons };
  if (env.HL_WORKFLOW_ARTIFACT_DIR) reasons.push('env artifact dir was invalid');
  ...
  if (fallback.length > 1) reasons.push('fallback is ambiguous; set HL_WORKFLOW_ARTIFACT_DIR');
