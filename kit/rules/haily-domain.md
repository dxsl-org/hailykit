...
Diagrams (Mermaid v11)                       → {skill:hl-visualize} --mermaid
Bulk OCR PDF/scans → Markdown (tiered cost, verify) → {skill:hc-ocr}
```
Disambiguate: batch of scanned PDFs/images needing tiered cost control and sample verification → `{skill:hc-ocr}` · single external doc or project docs, native-Read extraction → `{skill:hc-docs} extract` · turning transcribed/researched material into authored prose → `{skill:hl-write}`.
