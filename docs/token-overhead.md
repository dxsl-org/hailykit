...
Further diagnosis invalidated the behavioral reading of both earlier smoke outcomes. `requiredAnyOf` used an affirmed matcher, so correct prohibitions such as “never fabricate sources” could not satisfy the anti-fabrication group. Raw answers are intentionally digest-only, so the fallback failures cannot be reclassified semantically after the fact. The evaluator now separates affirmed requirements from `requiredNegatedAnyOf`, and the fixture uses explicit fallback-condition and role-order groups.

After the correction, an approved two-call read-only smoke compared `main@79a5f66` with candidate `2ccdbe6` (`manifestHash=04ea048c5a0018e266594cf5e0b9ddbf33bb06805c78747f83a8ce4e7b15b990`). Both arms failed only the same explicit role-order group, with no critical flags; candidate total tokens were `28,907` versus base `32,629` (`-3,722`, `-11.41%`). This is no observed regression under the agreed minimum gate, not behavioral-equivalence proof: the report remains inconclusive because one pair is underpowered and lacks frozen margin, completed calibration, private holdout, and complete provider footprints.
