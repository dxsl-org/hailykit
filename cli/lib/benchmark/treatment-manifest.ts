...
import type { BenchmarkCalibrationState, BenchmarkMarginRegistry, BenchmarkProvider, BenchmarkProvenance } from './types';
...
export interface ResolvedWorkflowManifest extends WorkflowTreatmentManifest { baseCommitSha: string; candidateCommitSha: string; fixtureRoot: string; }

const MANIFEST_KEYS = ['provider', 'tier', 'requestedModel', 'policy', 'provenance', 'liveEquivalent', 'budgetAcknowledged', 'budget', 'baseRef', 'candidateRef', 'fixtureRoot', 'fixturePaths', 'repeats', 'randomSeed', 'cliVersion', 'configSnapshotHash', 'componentClass', 'ablations', 'marginRegistry', 'calibration', 'evaluatorEvidenceHash', 'treatmentFiles'] as const;
...
export function resolveWorkflowManifest(repoRoot: string, value: WorkflowTreatmentManifest): ResolvedWorkflowManifest {
  ...
  return {
    ...
    requestedModel: reqString(input.requestedModel, 'workflow manifest.requestedModel'), policy, provenance, liveEquivalent,
