...
const SKILLS_MANIFEST = 'hailykit-installed-pi-skills.json';
const AGENTS_MANIFEST = 'hailykit-installed-pi-agents.json';
const VERSION_META = '.hailykit-pi-meta.json';
...
export class PiProvider extends BaseProvider {
  get name(): string { return 'pi'; }
  get label(): string { return 'Pi'; }
  globalDir(): string { return process.env['PI_CODING_AGENT_DIR'] ?? path.join(os.homedir(), '.pi', 'agent'); }
  protected _projectDirName(): string { return '.pi'; }
  ...
  private _skillsPaths(targetProviderDir: string): ManagedResourcePaths {
    return {
      ...
      markerPath: (name) => path.join(targetProviderDir, 'skills', name, '.hailykit-pi-skill.json'),
    };
  }
  ...
  private _agentsPaths(targetProviderDir: string): ManagedResourcePaths {
    return {
      ...
      markerPath: (name) => path.join(targetProviderDir, 'agents', `${name}.hailykit-pi-agent.json`),
    };
  }
  ...
  uninstall(providerDir: string): void {
    ...
    if (removedSkills > 0) console.log(`    Removed ${removedSkills} Pi skill(s)`);
    if (removedAgents > 0) console.log(`    Removed ${removedAgents} Pi agent(s)`);
