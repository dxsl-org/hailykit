...
function sampleObservation(arm: 'base' | 'candidate', tokens: number): BenchmarkObservation {
  return {
    ...
    providerExtensions: {}, legacy: { baselineEligible: null, attemptedComplete: null, actualPolicy: 'read_only', policySatisfied: true, coverage: null, hardChecksPassed: null, hardChecksTotal: null, finalAnswer: '<script>\u001b[31mboom</script>`x`', note: null, commitSha: 'abc' },
  };
}
...
test('report escapes HTML, markdown, ANSI, and control characters', () => {
  const manifest: BenchmarkManifest = {
    ...
    legacy: { attemptedComplete: null, baselineEligible: null, commitSha: null },
  };
...
test('report reaches GO only with matched live decision artifacts and confidence gates', () => {
  ...
  const rows = [1, 2, 3].flatMap((index) => {
    ...
    return (['base', 'candidate'] as const).map((arm) => {
      ...
      return {
        ...
        providerExtensions: { outputDigest: `${pairId}:${arm}`, evaluation: { deterministicComplete: true, criticalFlags: [], failedChecks: [], scopeDrift: false, unnecessaryWork: false, judgeEvidence: 'none', fixtureSplit: 'private-hash-only-holdout' } },
      };
    });
  });
  ...
  const report = buildBenchmarkReport(text, { minimumEffectivePairs: 2, providerFootprint: 'complete', holdoutManifest: { schemaVersion: 1, fixtureSetHash: 'suite-fh', promptCount: 3, containsRawPrompts: false }, holdoutArtifactText: text });
...
test('report rejects a hash-only claim that is not backed by evaluated private holdout rows', () => {
  const manifest: BenchmarkManifest = {
    ...
    calibration: { completedLiveBatches: 0, firstDecisionBatch: 3 }, snapshot: null, legacy: { attemptedComplete: true, baselineEligible: true, commitSha: 'abc' },
  };
